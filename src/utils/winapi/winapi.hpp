#ifndef CS2_PROJECT_WINAPI_HPP
#define CS2_PROJECT_WINAPI_HPP
...
namespace winapi
{
    ...
    public:
        ...
        __forceinline T get_exported_function( const hash32_t &module_hash, const hash32_t &export_hash )
        {
            ...
            auto export_directory = ( PIMAGE_EXPORT_DIRECTORY )( ( std::uint8_t * )module_base +
            ...
            for ( auto i = 0; i < export_directory->NumberOfNames; i++ ) {
                auto name = ( std::uint8_t * )module_base + ( ( std::uint32_t * )( ( std::uint8_t * )module_base +
                                                                                   export_directory->AddressOfNames ) )[ i ];

                if ( HASH( reinterpret_cast< const char * >( name ) ) == export_hash )
